...
export const handleWalletConnectAndDeployIslandContract = async () => {
  try {
    ...
    if (error?.code === 4001) {
      ...
      errorMessage = 'Unable to add Arbitrum Sepolia network. Please add it manually.';
...
async function deployContract(signer, provider) {
  try {
    ...
    if (!MAIN_LOGIC_CONTRACT_ADDRESS || !ethers.isAddress(MAIN_LOGIC_CONTRACT_ADDRESS)) {
    ...
    try {
      ...
      if (gasError.code === 'UNPREDICTABLE_GAS_LIMIT' || gasError.message?.includes('gas')) {
        try {
          deployedContract = await contractFactory.deploy(...constructorArgs, {
    ...
    try {
      ...
      // Don't fail the entire deployment for store issues
    }
    ...
    if (error?.message?.includes('insufficient funds')) {
      ...
      errorMessage = 'Gas estimation or execution failed. The contract might be too complex or have errors.';
    } else if (error?.message?.includes('revert')) {
      errorMessage = 'Contract deployment was reverted. Check constructor parameters and contract code.';
    } else if (error?.message?.includes('timeout')) {
      errorMessage = 'Deployment timed out. The transaction might still be pending on the network.';
    } else if (error?.message?.includes('bytecode') || error?.message?.includes('Invalid')) {

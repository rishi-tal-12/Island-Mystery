...
const solidityEnumToBuildingType = (
  enumVal: number
): Building["type"] | null => {
...
export default function IslandViewHex({ island, onBack }: IslandViewHexProps) {
  ...
  const { contract, signer, mainContract, islandContract } =
    useStoreContract() as any;
  ...
  const getAttackCooldown = () => {
    ...
    const nextAttackTime = new Date(
      lastAttackTime.getTime() + cooldownHours * 60 * 60 * 1000
    );
    const now = new Date();
  ...
  const fetchIslandState = async () => {
    try {
      ...
      if (typeof islandInstance.getStats === "function") {
        ...
        const attack = Number(
          attackBN.toString ? attackBN.toString() : attackBN
        );
        const defense = Number(
          defenseBN.toString ? defenseBN.toString() : defenseBN
        );
      ...
      if (typeof islandInstance.getTotalHexes === "function") {
        try {
          ...
          const totalHexCount = Number(
            totalHexesBN.toString ? totalHexesBN.toString() : totalHexesBN
          );
          ...
          for (let i = 0; i < updatedHexes.length; i++) {
            ...
            try {
              if (typeof islandInstance.hexes === "function") {
                ...
                const bTypeNum = Number(
                  bTypeRaw.toString ? bTypeRaw.toString() : bTypeRaw
                );
      ...
      // inside fetchIslandState, after you finish building updatedHexes
      let attackBonus = 0;
      let defenseBonus = 0;

      for (const hex of updatedHexes) {
        if (hex.building) {
          if (hex.building.type === "defense") {
            defenseBonus += 10 * hex.building.level;
          } else if (hex.building.type === "troop") {
            attackBonus += 10 * hex.building.level;
          }
        }
      }

      // finally update state with base stats + bonuses
      setStats((prev) => ({
  ...
  const handleHexClick = async (hex: Hex) => {
    ...
    if (selectedBuildingType && !hex.building) {
      const buildingType = BUILDING_TYPES.find(
        (bt) => bt.type === selectedBuildingType
      );
      ...
      if (
        !(
          resources.wheat >= buildingType.cost.wheat &&
          resources.gold >= buildingType.cost.gold
        )
      ) {
      ...
      if (
        islandInstance &&
        typeof islandInstance.placeBuilding === "function" &&
        signer
      ) {
        try {
          setTxInProgress(true);
          ...
          const tx = await islandInstance
            .connect(signer)
            .placeBuilding(hexIndex, enumVal);
          await tx.wait();
  ...
  const canAfford = (buildingType: Building["type"]) => {
    ...
    return (
      resources.wheat >= building.cost.wheat &&
        ...
        <div className="flex gap-3 mb-6">
          ...
          >
            ...
            {selectedBuildingType &&
              `(${
                BUILDING_TYPES.find((bt) => bt.type === selectedBuildingType)
                  ?.name
              })`}
          </Button>
            ...
            <div className="p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-orange-200 text-sm">
                  Next attack available in: {getAttackCooldown()}
                </p>
              </div>
            </div>
          </Card>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          ...
          >
            <div className="absolute top-2 left-2 bg-black/70 text-white px-2 py-1 rounded text-sm z-10">
              ...
              {hexes.filter((h) => h.isUnlocked).length}
            </div>
                  ...
                  <div className="w-full h-full flex items-center justify-center bg-white/10 backdrop-blur-sm">
                    ...
                    {hex.building ? (
                      <div className="text-center">
                        <img
                          src={
                            getBuildingTypeInfo(hex.building.type)?.icon ||
                            "/placeholder.svg"
                          }
